...
(async () => {
  ...
  /* ══════════════════════════════════════════════════════════
     ...
     Garmin's own Health API is a partner programme whose secret would
     have to live on this server, which would mean this server holding
     a credential that reads the whole account. It does not. The phone
     does the reading — Garmin Connect writes steps and sleep into the
     phone's health store, a Shortcut reads that and POSTs four numbers
     and a date here in the background — and the app collects them the
     next time it is opened.
     ══════════════════════════════════════════════════════════ */
  r = await hit('POST', '/v1/health/NIKO4821', { body: { steps: 8241 } });
  ok('posting a reading without the key is refused', r.status === 401, 'got ' + r.status);
  r = await hit('GET', '/v1/health/NIKO4821');
  ok('and reading them back needs the key too — this is not the board',
    r.status === 401, 'got ' + r.status);

  r = await hit('POST', '/v1/health/NIKO4821',
    { key: KEY, body: { steps: 8241, sleep: 7.4, water: 2.1, fuel: 2260 } });
  ok('a Shortcut with the key files four readings',
    r.status === 200 && (await r.json()).took === 4, 'got ' + r.status);
  r = await hit('GET', '/v1/health/NIKO4821', { key: KEY });
  let box = await r.json();
  ok('...against the server\u2019s own day when it names none',
    JSON.stringify(box[day(0)]) === '{"steps":8241,"sleep":7.4,"water":2.1,"fuel":2260}',
    JSON.stringify(box));

  /* MERGED, not replaced. Two automations — one for last night's sleep
     and one for the day's steps — write the same date, and the second
     must not take the first one's readings with it. */
  r = await hit('POST', '/v1/health/NIKO4821', { key: KEY, body: { steps: 9333 } });
  box = await (await hit('GET', '/v1/health/NIKO4821', { key: KEY })).json();
  ok('a second post merges into the day rather than replacing it',
    box[day(0)].steps === 9333 && box[day(0)].sleep === 7.4, JSON.stringify(box));

  /* DROPPED, NOT CLAMPED. A Shortcut handing over milliseconds of sleep
     instead of hours writes 27000000, and every figure drawn for the
     rest of the year is that one day. A clamped 24 is a lie that looks
     like a reading. Checked here as well as on the client because a bad
     reading stored is a bad reading served to every future open, and
     the client that filters it is not the client that put it in. */
  r = await hit('POST', '/v1/health/NIKO4821',
    { key: KEY, body: { sleep: 27000000, water: 1.9 } });
  box = await (await hit('GET', '/v1/health/NIKO4821', { key: KEY })).json();
  ok('a reading out of range is dropped and the one beside it is not',
    box[day(0)].sleep === 7.4 && box[day(0)].water === 1.9, JSON.stringify(box));
  r = await hit('POST', '/v1/health/NIKO4821', { key: KEY, body: { sleep: 27000000 } });
  ok('...and a post with nothing left in it is an error, not a silent 200',
    r.status === 400, 'got ' + r.status);

  /* THE SAME WINDOW AS EVERYTHING ELSE HERE, and it is the record's own
     — one function, because two copies of "which days count" is the
     pair that drifts. The client's backfill rule is stricter and is
     what actually decides; this only stops the store growing. */
  await hit('POST', '/v1/health/NIKO4821', { key: KEY, body: { steps: 10, on: day(-40) } });
  await hit('POST', '/v1/health/NIKO4821', { key: KEY, body: { steps: 20, on: day(-3) } });
  box = await (await hit('GET', '/v1/health/NIKO4821', { key: KEY })).json();
  ok('a day outside the window falls out, one inside it stays',
    !box[day(-40)] && box[day(-3)].steps === 20, JSON.stringify(Object.keys(box)));

  /* THE READ DOES NOT CLEAR. A destructive read loses the morning if
     the app is opened and closed before it files, and writing the same
     value twice is a no-op anyway. */
  box = await (await hit('GET', '/v1/health/NIKO4821', { key: KEY })).json();
  ok('...and reading them does not empty the inbox', box[day(0)].steps === 9333,
    JSON.stringify(box));

  r = await hit('GET', '/v1/health/NOSUCH99', { key: KEY });
  ok('an unclaimed code has no inbox to read', r.status === 401, 'got ' + r.status);
